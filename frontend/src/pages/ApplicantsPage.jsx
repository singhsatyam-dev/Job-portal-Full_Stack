...
const ApplicantsPage = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const [applicantsRes, jobRes] = await Promise.all([getApplicants(id), getJobById(id)]);
    ...
    <div className="page-container-md" style={{ paddingTop: "2.5rem", paddingBottom: "4rem" }}>
      ...
      <div className="glass fade-in px-8 py-7 mb-6">
        <div className="flex items-center gap-2.5 mb-1.5">
          ...
          <h1 className="font-extrabold text-2xl" style={{ fontFamily: "'Outfit', sans-serif", color: "var(--text-primary)" }}>
            Applicants
          </h1>
        </div>
        <p className="text-sm mb-3" style={{ color: "var(--text-muted)" }}>
          For: <span className="font-semibold" style={{ color: "var(--accent-light)" }}>{jobTitle}</span>
        </p>
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-semibold"
          style={{ background: "var(--accent-dim)", border: "1px solid var(--border-hover)", color: "var(--accent-light)" }}>
          <Users size={12} />
      ...
      {loading ? (
        ...
        <div className="glass p-16 text-center">
          ...
          <h3 className="text-lg font-semibold mb-2" style={{ color: "var(--text-secondary)" }}>No applicants yet</h3>
          <p className="text-sm" style={{ color: "var(--text-muted)" }}>Share your job listing to attract candidates.</p>
        </div>
      ) : (
            ...
            <div key={i} className="glass glass-hover px-6 py-5">
              <div className="flex flex-wrap gap-4 justify-between items-center">
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <div className="font-bold text-sm" style={{ color: "var(--text-primary)" }}>{applicant.name}</div>
                ...
                <div className="flex items-center gap-3.5">
                  ...
                  {applicant.resume ? (
                    <a href={`http://localhost:3000/uploads/resumes/${applicant.resume}`}
                      target="_blank" rel="noopener noreferrer" className="btn-secondary text-xs" style={{ padding: "7px 13px" }}>
